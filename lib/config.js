...
import { secretsEncryptionEnabled } from './secrets.js';
...
const SENSITIVE_KEYS = ['password', 'eskizPassword', 'telegram', 'token', 'newPassword', 'oldPassword', 'paymeKey', 'paymeTestKey'];
 ...
 *
 * Only the bearer token is accepted. `x-vercel-cron` / `x-vercel-signature` used to
 * pass on mere presence — anyone can send a header, so that was a bypass. Vercel Cron
 * itself sends `Authorization: Bearer <CRON_SECRET>` when the env var is set, so the
 * bearer check covers it without a header allowlist.
 */
export function cronRequestRejected(req) {
  const secret = process.env.CRON_SECRET;

  // No loopback exception any more: a "local" server talks to the production
  // database, and on 2026-09-11 a request from localhost ran the month's billing for
  // real. Local runs set CRON_SECRET in .env like everyone else.
  if (!secret) return 'CRON_SECRET sozlanmagan — vazifa bajarilmadi';
...
export function stripSettingSecrets(settings) {
  ...
  const { eskizPassword, telegram, paymeEndpointToken, ...safe } = hidePaymeSecrets(settings);
  return { ...safe, eskizPasswordSet: !!eskizPassword, telegramSet: !!telegram };
}

// Payme kalitlari hech kimga qaytarilmaydi — adminga ham. Ular faqat Payme
// kabinetidan olinadi va bir marta kiritiladi; brauzerga chiqishi uchun
// hech qanday sabab yo'q, qaytmasa XSS yoki ochiq qolgan sessiya ham ololmaydi.
export function hidePaymeSecrets(settings) {
  if (!settings) return settings;
  // telegramWebhookSecret ham shu yerda yashiriladi — u faqat server va Telegram o'rtasida.
  const { paymeKey, paymeTestKey, telegramWebhookSecret, ...safe } = settings;
  // `settingsEncryption` — SETTINGS_KEY o'rnatilganmi; Payme sahifasi shundan
  // ogohlantiradi, server esa usiz kalitni umuman saqlamaydi.
  return { ...safe, paymeKeySet: !!paymeKey, paymeTestKeySet: !!paymeTestKey, settingsEncryption: secretsEncryptionEnabled() };
}
