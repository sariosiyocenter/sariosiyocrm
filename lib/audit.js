...
const FIELD_LABELS = {
  ...
  kpiPercent: 'KPI foizi', schoolId: 'Filial', password: 'Parol',
  ...
  orgName: 'Markaz nomi', logo: 'Logotip', adminPhone: 'Admin telefoni', centerLocation: 'Markaz joylashuvi',
...
const OMIT_FIELDS = new Set([
  'id', 'createdAt', 'updatedAt', 'statusChangedAt', 'studentIds', 'descriptor', 'blocks', 'variants',
...
const SPECS = [
  ...
  {
    method: 'POST', path: '/api/students/:id/course-start', entity: 'student', action: 'update', recordId: 'id',
    describe: async ({ req, before, body }) => ({
      title: before?.name,
      summary: `Kursga kelgan sana: ${(await nameOfRecord('group', req.body?.groupId)) || '—'} → ${req.body?.date}`
        + (body?.balanceDelta ? ` · hisob ${money(body.balanceDelta)} so'm` : ''),
      changes: [],
    }),
  },
  {
    method: 'POST', path: '/api/students/:id/custom-price', entity: 'student', action: 'update', recordId: 'id',
    describe: async ({ req, before }) => ({
      title: before?.name,
      summary: `Shaxsiy narx: ${(await nameOfRecord('group', req.body?.groupId)) || '—'} → `
        + (req.body?.price === null || req.body?.price === undefined || req.body?.price === ''
          ? 'standart narx' : `${money(req.body.price)} so'm`),
      changes: [],
    }),
  },
  {
  ...
  },
  {
    ...
    describe: async ({ req, body }) => ({ title: await nameOfRecord('group', req.body?.groupId), summary: `${req.body?.date} davomati ota-onalarga yuborildi (${body?.sent ?? 0} ta)`, link: `/courses/${parseInt(req.body?.groupId)}` }),
  },
  ...
  ...crud('/api/exams', 'exam'),
  { method: 'POST', path: '/api/exams/:id/assignments', entity: 'exam', action: 'update', recordId: 'id', describe: async ({ before, req }) => ({ title: before?.name, summary: `Kursga biriktirildi: ${(await nameOfRecord('group', req.body?.groupId)) || '—'}`, changes: [] }) },
  ...
  {
    method: 'POST', path: '/api/exam-results', entity: 'examResult', action: 'create',
    describe: async ({ after }) => ({
      title: await nameOfRecord('student', after?.studentId),
      summary: `${(await nameOfRecord('exam', after?.examId)) || ''} · ${after?.score} ball`,
      link: after?.studentId ? `/students/${after.studentId}` : null,
    }),
  },
  ...
  { method: 'POST', path: '/api/sms/attendance', entity: 'message', action: 'action', describe: async () => ({ title: 'Davomat SMS' }) },
