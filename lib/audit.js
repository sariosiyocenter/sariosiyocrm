...
const SPECS = [
  ...
  {
    method: 'PUT', path: '/api/qarz-xabari', entity: 'message', action: 'update',
    describe: async ({ body }) => {
      const s = body?.sozlama || {};
      return {
        title: 'Qarz eslatmasi sozlamasi',
        summary: `Avtomatik: ${s.yoqilgan ? 'yoqilgan' : "o'chirilgan"} · har oy ${s.kun}-kuni ${s.soat} dan${s.takror ? `, keyin har ${s.takror} kunda` : ''}, ko'pi bilan ${s.maks} marta · ${s.minQarz} so'mdan · kanal: ${s.kanal} · kimga: ${s.kimga} · shablon: ${(await nameOfRecord('messageTemplate', s.shablonId)) || '—'}`,
        link: '/messaging', changes: [],
      };
    },
  },
  {
    method: 'POST', path: '/api/qarz-xabari/yubor', entity: 'message', action: 'action',
    describe: async ({ body }) => ({ title: 'Qarzdorlarga eslatma', summary: `${body?.yaratildi ?? 0} ta o'quvchiga${body?.takror ? ` (${body.takror} tasiga 10 daqiqa ichida yuborilgan — takrorlanmadi)` : ''}`, link: '/messaging', changes: [] }),
  },
  { method: 'POST', path: '/api/qarz-xabari/navbat', skip: true },   // yuborish jarayoni (yozuvlar /yubor da)
  {
    method: 'POST', path: '/api/qarz-xabari/:id/qayta', entity: 'message', action: 'action',
    describe: async ({ body }) => ({ title: await nameOfRecord('student', body?.studentId), summary: `Qarz eslatmasi qayta yuborildi · ${body?.holat || ''}${body?.sabab ? ` · ${body.sabab}` : ''}`, link: '/messaging', changes: [] }),
  },
  {
    method: 'POST', path: '/api/qarz-xabari/sinov', entity: 'message', action: 'action',
    describe: async ({ req }) => ({ title: 'Qarz eslatmasi — sinov SMS', summary: `Raqam: ${String(req.body?.telefon || '').slice(0, 20)}`, link: '/messaging', changes: [] }),
  },
  {
    method: 'POST', path: '/api/qarz-javob/:id/yop', entity: 'message', action: 'action',
    describe: async ({ req, body }) => ({ title: await nameOfRecord('student', body?.studentId), summary: `Ota-ona "to'laganman" javobi: ${req.body?.natija === 'kiritildi' ? "to'lov kiritildi" : "qarz to'g'ri"}${req.body?.izoh ? ` · ${String(req.body.izoh).slice(0, 120)}` : ''}`, link: '/messaging', changes: [] }),
  },
