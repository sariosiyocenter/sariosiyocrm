...
const SPECS = [
  ...
  },
  {
    ...
    describe: async ({ before, body }) => ({
      title: before?.name,
      summary: `Kitobcha kaliti (qo'lda): ${(body?.kalitlar || []).map(k => `${k.code} ${k.toldirilgan}/${k.jami}`).join(', ')}${body?.qaytaHisoblandi ? ` · ${body.qaytaHisoblandi} ta natija qayta hisoblandi` : ''}`,
      link: `/exams/${before?.id}`, changes: [],
    }),
  },
