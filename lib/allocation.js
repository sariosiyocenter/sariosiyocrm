...
export const RULES = ['eski', 'teng', 'qarz', 'foiz'];

/**
 * O'quvchi uchun amaldagi qoida (egasi, 2026-09-23: "поровну ... 50 на 50
 * ... для этого ученика каждый месяц из баланса снимается"):
 *   - kartochkada "Balans taqsimoti"da qo'lda foiz berilgan bo'lsa — o'sha;
 *   - aks holda TENG: balansdagi pul shu oyning kurs hisoblariga teng bo'linadi.
 * Server ham, brauzer ham shu funksiya bilan hisoblaydi — raqamlar bir xil.
 */
export function shareOpts(payShare) {
  if (payShare && typeof payShare === 'object' && RULES.includes(payShare.rule)) {
    return payShare.rule === 'foiz'
      ? { rule: 'foiz', weights: (payShare.weights && typeof payShare.weights === 'object') ? payShare.weights : {} }
      : { rule: payShare.rule };
  }
  return { rule: 'teng' };
}
...
/**
 * Kartochkadan kelgan qiymatni tekshirish. Qaytaradi: null (markaz qoidasi),
 * { rule[, weights] } yoki false (noto'g'ri).
 */
export function normalizePayShare(v) {
  if (v === null || v === undefined || v === '' || (typeof v === 'object' && !v.rule)) return null;
  if (typeof v !== 'object' || !RULES.includes(v.rule)) return false;
  if (v.rule !== 'foiz') return { rule: v.rule };
  const weights = {};
  let jami = 0;
  for (const [k, w] of Object.entries(v.weights || {})) {
    const n = Number(w);
    if (!/^\d+$/.test(k) || !Number.isFinite(n) || n < 0 || n > 100) return false;
    if (n > 0) { weights[k] = Math.round(n * 100) / 100; jami += weights[k]; }
  }
  if (Math.abs(jami - 100) > 0.01) return false;
  return { rule: 'foiz', weights };
}
...
export function allocate(rows, opts = {}) {
  // Bir nechta kursdagi o'quvchida balansdagi pul qanday bo'linadi (shareOpts:
  // standart teng, kartochkada qo'lda foiz). 'eski' — eski navbat, motor sinovlari uchun.
  const rule = RULES.includes(opts.rule) ? opts.rule : 'eski';
  // 'foiz' — o'quvchining kartochkasida qo'lda berilgan ulushlar:
  // { "<kurs id>": 70, "<kurs id>": 30 }. Ulushi yo'q kurs 0 hisoblanadi.
  const ulushlar = (opts.weights && typeof opts.weights === 'object') ? opts.weights : {};
  const vazn = (b) => {
    if (rule === 'foiz') { const w = Number(ulushlar[String(b.groupId)]); return Number.isFinite(w) && w > 0 ? w : 0; }
    return 1;
  };
  ...
  const ulush = (list, amount) => {
    ...
    while (left > 0 && faol.length) {
      ...
      // Ulushlar: teng — 1:1, foiz — qo'lda berilgan, qarz — qolgan hisob.
      // Faol kurslarning birortasida ham ulush bo'lmasa (masalan foiz
      // berilmagan yangi kurs qolgan) — teng bo'linadi, pul turib qolmaydi.
      let vaznlar = faol.map(b => (rule === 'qarz' ? qoldiq(b) : vazn(b)));
      if (!vaznlar.some(v => v > 0)) vaznlar = faol.map(() => 1);
      const jamiVazn = vaznlar.reduce((s, v) => s + v, 0);
      const oldin = left;
      faol.forEach((b, i) => ber(b, Math.min(qoldiq(b), Math.floor(oldin * vaznlar[i] / jamiVazn))));
      faol = faol.filter(b => qoldiq(b) > 0);
      if (left === oldin) {
        // Yaxlitlash sabab hech kimga tegmadi (qolgan so'mlar chelaklar sonidan
        // kam) — 1 so'mdan navbat bilan, ulushi kattasidan boshlab.
        const tartib = faol.map((b, i) => ({ b, v: rule === 'qarz' ? qoldiq(b) : vazn(b), i }))
          .sort((x, y) => y.v - x.v || x.i - y.i);
        for (const { b } of tartib) { if (left <= 0) break; ber(b, 1); }
        faol = faol.filter(b => qoldiq(b) > 0);
      }
    }
    return out;
