...
async function xodimTahriri(req) {
  ...
  const joriy = id && await prisma.user.findUnique({
    where: { id },
    select: { name: true, phone: true, phone2: true, photo: true, position: true, salary: true, role: true, email: true, status: true, workDays: true, kpiPercent: true, schoolId: true },
  });
  const talab = maydonTalablari(req.body, joriy, {
...
// SMS pullik: kanal "faqat Telegram" bo'lmasa — xabar yuborish ruxsati ham kerak.
r('POST', '/api/attendances/notify', {
  talab: req => [T('kurslar.davomat'), ...(req.body?.kanal !== 'TELEGRAM' ? [T('xabarlar.yuborish')] : [])],
});
// Davomat va to'lov xabari kartalari Xabarlar → Avtomatik tabida (2026-09-27):
// avtomatik qoidalar ruxsati ham yetadi.
r('GET', '/api/davomat-xabari', { k: ['kurslar.davomat', 'xabarlar.shablon', 'xabarlar.avto'], d: 1 });
r('PUT', '/api/davomat-xabari', { k: ['xabarlar.shablon', 'xabarlar.avto'], d: 2 });
// To'lov xabari (services/tolovXabari.js): sozlama — shablon ruxsati,
// qayta yuborish va sinov SMS — yuborish ruxsati.
r('GET', '/api/tolov-xabari', { k: ['xabarlar.shablon', 'xabarlar.yuborish', 'xabarlar.avto'], d: 1 });
r('PUT', '/api/tolov-xabari', { k: ['xabarlar.shablon', 'xabarlar.avto'], d: 2 });
r('POST', '/api/tolov-xabari/sinov', T('xabarlar.yuborish'));
r('POST', '/api/tolov-xabari/:id/qayta', T('xabarlar.yuborish'));
// Qarz eslatmasi (services/qarzXabari.js): sozlama — Avtomatik qoidalar;
// qarzdorlar ro'yxati va yuborish — Oylik nazorat ("qarzdorlarga xabar")
// yoki xabar yuborish ruxsati; ota-ona javobini yopish — to'lov bilan
// ishlaydigan xodim ham.
r('GET', '/api/qarz-xabari', { k: ['xabarlar.avto', 'moliya.oylik', 'xabarlar.yuborish'], d: 1 });
r('PUT', '/api/qarz-xabari', T('xabarlar.avto'));
r('GET', '/api/qarz-xabari/qarzdorlar', { k: ['moliya.oylik', 'xabarlar.avto', 'xabarlar.yuborish'], d: 1 });
r('POST', '/api/qarz-xabari/yubor', { k: ['moliya.oylik', 'xabarlar.yuborish'], d: 2 });
r('POST', '/api/qarz-xabari/navbat', { k: ['moliya.oylik', 'xabarlar.yuborish'], d: 2 });
r('POST', '/api/qarz-xabari/sinov', T('xabarlar.yuborish'));
r('POST', '/api/qarz-xabari/:id/qayta', { k: ['moliya.oylik', 'xabarlar.yuborish'], d: 2 });
r('POST', '/api/qarz-javob/:id/yop', { k: ['moliya.oylik', 'oquvchilar.tolov', 'xabarlar.avto'], d: 2 });
r('GET', '/api/qarz-javob/:id/rasm', { k: ['moliya.oylik', 'oquvchilar.tolov', 'xabarlar.avto'], d: 1 });
r('GET', '/api/qarz-javob/soni', { k: ['moliya.oylik', 'oquvchilar.tolov', 'xabarlar.avto'], d: 1 });
...
r('GET', '/api/tolov-tasdiq/takror', T('oquvchilar.tolov'));
...
// Imtihonlar (routes/imtihon.js). Kalit — alohida bo'lim: ustoz imtihonni
// tuzsa ham, to'g'ri javoblarni ko'rmasligi mumkin.
const IMTIHON_KORISH = { k: ['imtihonlar.imtihon', 'imtihonlar.natija'], d: 1 };
r('GET', '/api/questions', T('imtihonlar.savollar', 1));
r('GET', '/api/questions/meta', { k: ['imtihonlar.savollar', 'imtihonlar.imtihon'], d: 1 });
r('GET', '/api/questions/:id', T('imtihonlar.savollar', 1));
...
r('GET', '/api/passages', T('imtihonlar.savollar', 1));
r('POST', '/api/passages', T('imtihonlar.savollar'));
r('PUT', '/api/passages/:id', T('imtihonlar.savollar'));
r('DELETE', '/api/passages/:id', T('imtihonlar.ochirish'));
r('GET', '/api/exams', IMTIHON_KORISH);
r('GET', '/api/exams/history', T('imtihonlar.natija', 1));
r('GET', '/api/exams/:id', IMTIHON_KORISH);
r('POST', '/api/exams', T('imtihonlar.imtihon'));
r('PUT', '/api/exams/:id', T('imtihonlar.imtihon'));
r('DELETE', '/api/exams/:id', T('imtihonlar.ochirish'));
r('POST', '/api/exams/:id/copy', T('imtihonlar.imtihon'));
r('GET', '/api/exams/:id/assignments', IMTIHON_KORISH);
r('POST', '/api/exams/:id/assignments', T('imtihonlar.imtihon'));
r('DELETE', '/api/exams/:id/assignments/:id', T('imtihonlar.imtihon'));
r('GET', '/api/exams/:id/check', T('imtihonlar.imtihon', 1));
r('POST', '/api/exams/:id/lock', T('imtihonlar.imtihon'));
r('POST', '/api/exams/:id/unlock', T('imtihonlar.imtihon'));
r('GET', '/api/exams/:id/booklets', T('imtihonlar.imtihon', 1));
r('GET', '/api/exams/:id/key', T('imtihonlar.kalit', 1));
r('PUT', '/api/exams/:id/key', T('imtihonlar.kalit'));
r('PUT', '/api/exams/:id/manual-key', T('imtihonlar.kalit'));
r('GET', '/api/exams/:id/participants', T('imtihonlar.imtihon', 1));
r('POST', '/api/exams/:id/seating', T('imtihonlar.imtihon'));
r('GET', '/api/exams/:id/seats', IMTIHON_KORISH);
r('POST', '/api/exams/:id/guests', T('imtihonlar.imtihon'));
r('POST', '/api/exams/:id/admit-cards', T('imtihonlar.imtihon'));
// Tashqi qatnashchini lidga aylantirish — lidlar bilan ishlaydigan xodim.
r('POST', '/api/exams/:id/guests/leads', { talab: async () => [T('lidlar.royxat'), T('imtihonlar.imtihon', 1)] });
// Keldi/kelmadi — tuzuvchi ham, skaner qiluvchi ham; o'rinni ko'chirishni
// faqat tuzuvchi (imtihonlar.imtihon) qiladi — routes/imtihon.js ichida tekshiriladi.
r('PUT', '/api/exams/:id/seats/:id', { k: ['imtihonlar.imtihon', 'imtihonlar.natija'], d: 2 });
r('DELETE', '/api/exams/:id/seats/:id', T('imtihonlar.imtihon'));
r('POST', '/api/exams/:id/scans', T('imtihonlar.natija'));
r('GET', '/api/exams/:id/results', T('imtihonlar.natija', 1));
r('GET', '/api/exam-results/:id', T('imtihonlar.natija', 1));
r('PUT', '/api/exam-results/:id/review', T('imtihonlar.natija'));
r('POST', '/api/exams/:id/recalculate', T('imtihonlar.natija'));
r('GET', '/api/exams/:id/analysis', T('imtihonlar.natija', 1));
r('GET', '/api/exams/:id/publish-summary', T('imtihonlar.natija', 1));
r('POST', '/api/exams/:id/publish', T('imtihonlar.elon'));
r('POST', '/api/exams/:id/notify', T('imtihonlar.elon'));
r('GET', '/api/exams/:id/leaderboard', IMTIHON_KORISH);
r('GET', '/api/exam-results', { k: ['imtihonlar.natija', 'oquvchilar.ballar'], d: 1 });
r('DELETE', '/api/exam-results/:id', T('imtihonlar.ochirish'));
// AI (routes/imtihonAI.js): savol bilan ishlovchi va natijani tekshiruvchi.
r('GET', '/api/ai/holat', { k: ['imtihonlar.savollar', 'imtihonlar.natija'], d: 1 });
r('POST', '/api/questions/ai/import', T('imtihonlar.savollar'));
r('POST', '/api/questions/:id/ai/yechim', T('imtihonlar.savollar'));
r('POST', '/api/questions/:id/ai/klon', T('imtihonlar.savollar'));
r('POST', '/api/questions/:id/ai/tarjima', T('imtihonlar.savollar'));
r('POST', '/api/exam-results/:id/ai/baho', T('imtihonlar.natija'));
...
r('PUT', '/api/logistics/tarif', T('logistika.reja'));
...
r('PUT', '/api/logistics/day', T('logistika.reja'));
r('POST', '/api/logistics/plans/:id/holat', T('logistika.reja'));
...
r('GET', '/api/sms/navbat', T('xabarlar.tarix', 1));
...
r('POST', '/api/messaging/templates/:id/eskiz', T('xabarlar.shablon'));
