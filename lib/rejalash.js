 ...
 * @returns {any[][]}
 */
export function sektorlarga(markaz, uylar, olchamlar) {
  ...
  for (const olcham of olchamlar) {
    if (i >= tartib.length) break;
    const bolak = tartib.slice(i, i + olcham);
    i += bolak.length;
    if (bolak.length) bolaklar.push(bolak);
  }
  // Sig'im yetmay qolganlari oxirgi bo'lakka emas, alohida qaytariladi.
  const qolgan = tartib.slice(i);
...
export function reyalarniTuzish({ markaz, oquvchilar, mashinalar, direction = 'KETISH', startTime = '07:30', rejim = 'tez' }) {
  ...
  if (rejim === 'arzon') {
    ...
    const sektor = sektorlarga(markaz, uylar, ulush);
