...
export function bankYetarliligi({ blocks, settings, bank }) {
  ...
  const yaroqli = (bank || []).filter(q => (q.status || 'faol') === 'faol' && (!s.language || (q.language || 'uz') === s.language) && !savolXatosi(q));
  return (blocks || []).map(b => ({
    blok: b.subject,
    qoidalar: (b.topicRules || []).map(r => {
      const tur = turi(r.type);
      const qiyinlik = parseInt(r.difficulty) || 0;
      const bor = yaroqli.filter(q =>
        teng(q.subject, b.subject) &&
        (!String(r.topic || '').trim() || teng(q.topic, r.topic)) &&
        turi(q.type) === tur &&
        (!qiyinlik || Number(q.difficulty) === qiyinlik)).length;
      const kerak = (Math.max(0, parseInt(r.count) || 0)) * kopaytma;
      return { mavzu: r.topic || '', tur, kerak, bor, yetadi: bor >= kerak };
    }),
  }));
}
 ...
 */
export function savolTahlili({ variants, natijalar }) {
  ...
  for (const r of natijalar || []) {
    ...
    for (const it of v.items) {
      if (!stat.has(it.q)) stat.set(it.q, { q: it.q, t: it.t, b: it.b, jami: 0, togri: 0, bosh: 0, tanlov: {}, yJami: 0, yTogri: 0, qJami: 0, qTogri: 0 });
      const st = stat.get(it.q);
...
export function natijaXabari(shablon, m) {
  ...
  const boshlar = ['bloklar', 'orin', 'havola', 'markaz'].filter(k => !String(m[k] ?? '').trim());
