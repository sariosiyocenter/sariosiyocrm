...
function kalitlarniTozala(v) {
  ...
  for (const [kalit, qiymat] of Object.entries(v && typeof v === 'object' && !Array.isArray(v) ? v : {})) {
    ...
    out[kalit] = qiymat.slice(0, 500).map(x => String(x ?? '').trim().toUpperCase().slice(0, 60));
  }
  return out;
}
...
export function kalitdanVariantlar({ blocks, scoring = 'blok', settings }) {
  ...
  for (const { kalit } of kalitToplamlari(s)) {
    ...
    for (const q of tuz) {
      ...
      items.push({ n: q.n, q: null, b: q.b, t: q.t, p: q.p, ...(q.t === 'yopiq' ? { m: asl } : {}), ...r.el });
    }
