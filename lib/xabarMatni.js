...
//   {oxirgi_tolov} {imtihon_oylik}
//
// {imtihon_oylik} — o'tgan oyda e'lon qilingan imtihonlar natijasi (oylik
// hisobot qoidasi, routes/imtihon.js `oylikImtihonHisoboti`).
//
// {oxirgi_tolov} — o'quvchidan oxirgi qabul qilingan to'lov summasi (egasi,
// 2026-09-24). To'lov xabarida — aynan shu to'lov; boshqa xabarlarda
// yuborishdan oldin bazadan olinadi (`student.lastPaymentAmount`).
//
...
//
// Summa har doim bir xil ko'rinishda: "300,000". Ilgari toLocaleString()
// serverning mintaqa sozlamasiga qarab o'zgarardi (Vercel: "300,000", lokal:
// "300 000"), Eskiz esa SMS matnini tasdiqlangan shablon bilan harfma-harf
// solishtiradi. "192,308" ko'rinishi Eskizdan o'tgani tasdiqlangan.
const summa = (n) => Math.round(Number(n) || 0).toLocaleString('en-US');
...
export function fillTemplate(body, student, groupsForStudent, school) {
  ...
  const paymentAmount = student.customPaymentAmount !== undefined ? summa(student.customPaymentAmount) : '';
  const dailyScore = student.customDailyScore !== undefined ? String(student.customDailyScore) : '';
  const oxirgiTolovSumma = student.customPaymentAmount ?? student.lastPaymentAmount;
  const oxirgiTolovMatni = oxirgiTolovSumma !== undefined && oxirgiTolovSumma !== null
    ? summa(oxirgiTolovSumma) : '0';
  ...
  return String(body || '')
    ...
    .replace(/\{qarz\}/gi, summa(debt))
    .replace(/\{balans\}/gi, summa(balance))
    ...
    .replace(/\{markaz\}/gi, school?.orgName || school?.name || '')
    ...
    .replace(/\{oxirgi_tolov\}/gi, oxirgiTolovMatni)
    .replace(/\{bahosi\}/gi, dailyScore)
    .replace(/\{imtihon_oylik\}/gi, student.oylikImtihon || '');
}

/** Matnda {testnatijasi} bormi. */
export const testNatijasiKerak = (body) => /\{testnatijasi\}/i.test(String(body || ''));

/** Matnda {oxirgi_tolov} bormi. */
export const oxirgiTolovKerak = (body) => /\{oxirgi_tolov\}/i.test(String(body || ''));

/** Kassaga tushgan pul: musbat, hisob yozuvi emas (src/lib/money.ts isCashIncome bilan bir xil). */
export const kirimmi = (p) => Number(p?.amount) > 0 && p?.type !== 'Chegirma' && p?.type !== 'Oylik';

/**
 * O'quvchining oxirgi qabul qilingan to'lovi: sana bo'yicha eng yangisi, bir
 * kunda bir nechta bo'lsa — keyin kiritilgani. Server ham, Xabarlar sahifasi
 * ham shu funksiyadan foydalanadi.
 */
export function oxirgiTolov(payments) {
  let eng = null;
  for (const p of payments || []) {
    if (!kirimmi(p)) continue;
    if (!eng) { eng = p; continue; }
    const d = String(p.date || '').localeCompare(String(eng.date || ''));
    if (d > 0 || (d === 0 && (p.id || 0) > (eng.id || 0))) eng = p;
  }
  return eng;
}
