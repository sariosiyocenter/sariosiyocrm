        ...
        switch (activeReport) {
            ...
            default: return (
                <div className="flex-1 flex flex-col items-center justify-center p-20 text-center opacity-60">
                    <div className="w-24 h-24 bg-white/5 border border-white/5 rounded-[2.5rem] flex items-center justify-center text-white/10 mb-8">
                        <FileText size={40} />
                    </div>
                    <p className="text-[10px] font-extrabold text-[#4a5568] uppercase tracking-widest">Davom etish uchun yuqoridagi filtrlardan bittasini tanlang</p>
                </div>
            );
        }
    };
    ...
    return (
        <div className="space-y-8 py-4 animate-in fade-in duration-700">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-extrabold text-white uppercase tracking-tight">Hisobotlar Tizimi</h1>
                    <p className="text-[10px] font-bold text-[#4a5568] mt-2 uppercase tracking-widest">Markaz statistikasi va ma'lumotlar tahlili</p>
                </div>
                
                <div className="flex flex-wrap items-center gap-4 bg-white/[0.03] backdrop-blur-md p-4 rounded-[1.75rem] border border-white/5 shadow-sm w-full lg:w-auto">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-white/5 border border-white/5 rounded-lg flex items-center justify-center text-[#4a5568]">
                            <Calendar size={14} />
                        </div>
                        <input
                            type="date"
                            ...
                            className="bg-white/5 border border-white/5 px-4 py-2 rounded-lg text-[9px] font-extrabold uppercase tracking-widest text-white/70 outline-none focus:border-violet-500 transition-all shadow-inner w-32"
                        />
                        <span className="text-white/20 font-extrabold text-xs">-</span>
                        <input
                            type="date"
                            value={endDate}
            ...
            <div className="flex flex-wrap gap-3 bg-[#141c27] p-6 rounded-[2rem] border border-white/5 shadow-xl shadow-black/20 animate-in slide-in-from-top-4 duration-500">
                {REPORT_TYPES.map((report, idx) => (
                    <button
                        ...
                        className={`flex items-center gap-3 px-5 py-3 rounded-2xl border text-left transition-all group scale-in duration-300 ${
                            ...
                            : 'bg-white/5 border-white/5 text-[#4a5568] hover:border-violet-500/50 hover:bg-white/[0.03]'
                        }`}
                        style={{ animationDelay: `${idx * 30}ms` }}
                    >
                        <div className={`p-2 rounded-xl transition-colors ${activeReport === report.id ? 'bg-white/20' : 'bg-white/5 border border-white/5 shadow-sm'}`}>
                        ...
                        <span className={`text-[10px] font-extrabold uppercase tracking-widest ${activeReport === report.id ? 'text-white' : 'text-white/60'}`}>
                            {report.label}
                        </span>
                    </button>
                ))}
            ...
            <div className="bg-[#141c27] rounded-[3rem] shadow-2xl shadow-black/20 border border-white/5 overflow-hidden min-h-[500px] animate-in slide-in-from-bottom-6 duration-700">
                <div className="px-10 py-8 border-b border-white/5 bg-white/[0.03] flex items-center justify-between">
