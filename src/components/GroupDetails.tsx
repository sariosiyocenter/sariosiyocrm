...
export default function GroupDetails() {
    ...
    return (
        <div className="space-y-6 pb-12 animate-in fade-in duration-700">
            <button onClick={() => navigate('/groups')} className="flex items-center gap-2 text-[#4a5568] hover:text-sky-600 dark:hover:text-sky-400 transition-all text-[10px] font-bold uppercase tracking-widest group">
                <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
                Orqaga qaytish
            </button>

            <div className="bg-[#141c27] rounded-3xl shadow-sm border border-white/5 overflow-hidden transition-colors">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                        <div>
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <span className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">ID: #{group.id}</span>
                            </div>
                            <h1 className="text-4xl font-extrabold text-white uppercase tracking-tight">{group.name}</h1>
                            <div className="flex items-center gap-5 mt-4 text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">
                                <span className="flex items-center gap-2 px-3 py-1.5 bg-[#141c27] rounded-lg border border-white/5 shadow-sm">
                                    <BookOpen size={14} className="text-sky-500" />
                                    {course?.name || "Noma'lum kurs"}
                                </span>
                            </div>
                        </div>
                        <div className="flex items-center gap-5 bg-white/[0.03] border border-white/5 shadow-xl shadow-sky-500/5 p-5 pr-8 rounded-[2rem] shrink-0 transition-all hover:scale-105 group relative">
                            {isEditingInfo ? (
                                <div className="flex flex-col gap-2 min-w-[200px]">
                                    <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest pl-1">O'qituvchi</label>
                                    <select 
                                        value={editForm.teacherId}
                                        onChange={e => setEditForm({ ...editForm, teacherId: Number(e.target.value) })}
                                        className="w-full px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-xs font-bold focus:border-sky-500 outline-none"
                                    >
                                        {teachers.map(t => (
                                ...
                                <>
                                    ...
                                    <div>
                                        <p className="text-[10px] font-bold text-[#4a5568] mb-1.5 uppercase tracking-widest">O'qituvchi</p>
                                        <p className="text-lg font-bold text-white uppercase tracking-tight group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">{teacher?.name}</p>
                                    </div>
                                </>
                            )}
                        </div>
                ...
                <div className="flex px-4 bg-white/[0.03] border-b border-white/5 gap-2 overflow-x-auto custom-scrollbar">
                ...
                <div className="p-8 md:p-10">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                <div className="lg:col-span-2 space-y-8">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-xl font-bold text-white uppercase tracking-tight">O'quvchilar</h3>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                        {groupStudents.map(s => (
                                            <div key={s.id} onClick={() => navigate(`/students/${s.id}`)} 
                                                className="group bg-[#141c27] border border-white/5 p-5 rounded-[2rem] transition-all cursor-pointer hover:border-sky-300 dark:hover:border-sky-500 hover:shadow-xl hover:shadow-sky-500/5 hover:-translate-y-1">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-sm font-bold text-white group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors truncate uppercase tracking-tight">{s.name}</p>
                                                            <p className="text-[10px] font-bold text-[#4a5568] mt-1 uppercase tracking-widest leading-none">{s.phone}</p>
                                                        </div>
                                                    </div>
                                        ...
                                        {groupStudents.length === 0 && (
                                            <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-[2.5rem] bg-white/[0.03]">
                                                <Users className="mx-auto h-16 w-16 text-white/10 mb-5" />
                                                <p className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">Bu guruhda o'quvchilar yo'q</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="space-y-8">
                                    <div className="flex items-center justify-between ml-1">
                                        <h3 className="text-sm font-bold text-[#4a5568] uppercase tracking-widest">Ma'lumotlar</h3>
                                        {!isEditingInfo ? (
                                            <button 
                                    ...
                                    <div className="bg-[#141c27] border border-white/5 rounded-[2.5rem] p-6 space-y-6 shadow-sm transition-colors">
                                        {isEditingInfo ? (
                                            <>
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">Kunlar</label>
                                                    <select 
                                                        value={editForm.days}
                                                        onChange={e => setEditForm({ ...editForm, days: e.target.value })}
                                                        className="w-full px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-xs font-bold outline-none"
                                                    >
                                                ...
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">Vaqt</label>
                                                    <div className="flex items-center gap-2">
                                                        <input 
                                                            type="time" 
                                                            value={editForm.startTime}
                                                            onChange={e => setEditForm({ ...editForm, startTime: e.target.value })}
                                                            className="flex-1 px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-xs font-bold outline-none"
                                                        />
                                                ...
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">Xona</label>
                                                    <select 
                                                        value={editForm.room}
                    ...
                    {activeTab === 'yoqlama' && (
                        <div className="space-y-8 animate-in duration-500 zoom-in-95">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-white/[0.03] border border-white/5 p-8 rounded-[2rem] transition-colors relative overflow-hidden">
                                {!isDayValid(group.days, selectedDate) && (
                                    <div className="absolute top-0 left-0 right-0 h-1 bg-amber-500/50" />
                                )}
                                <div>
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight">Davomat belgilash</h3>
                                    {!isDayValid(group.days, selectedDate) ? (
                                        ...
                                        <p className="text-[10px] font-bold text-[#4a5568] mt-2 uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                                    <div className="flex items-center gap-2 bg-[#141c27] p-1.5 rounded-[1.5rem] border border-white/5 shadow-sm">
                                        <button 
                                            onClick={() => setSelectedDate(prev => getValidDate(prev, group.days, 'prev'))}
                                            className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-white/5 text-gray-400 hover:text-sky-600 transition-all"
                                            title="Oldingi dars"
                                        >
                                            <ArrowLeft size={18} />
                                        </button>
                                        ...
                                        <div className="relative group">
                                            <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)}
                                                className={`px-4 py-2 bg-transparent text-xs font-bold uppercase tracking-widest outline-none border-none ${!isDayValid(group.days, selectedDate) ? 'text-amber-600' : 'text-white'} transition-all`} />
                                        </div>
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                                <div className="xl:col-span-1 border-r-0 xl:border-r border-dashed border-white/5 pr-0 xl:pr-8">
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight mb-6">Oylik Grafik</h3>
                                ...
                                <div className="xl:col-span-2 space-y-6">
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight">Davomat Tarixi (Matritsa)</h3>
                            ...
                            {false && (
                                <div className="bg-[#141c27] rounded-[2.5rem] overflow-hidden shadow-sm border border-white/5 transition-colors">
                                <div className="overflow-x-auto custom-scrollbar">
                                    <table className="w-full text-left border-collapse min-w-[500px]">
                                        <thead>
                                            <tr className="bg-white/[0.03] border-b border-white/5">
                                                <th className="p-6 text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">O'quvchi</th>
                                                <th className="p-6 text-center text-[10px] font-bold text-[#4a5568] uppercase tracking-widest w-72">Holati</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-white/5">
                                            ...
                                            {groupStudents.length === 0 && (
                                                <tr>
                                                    <td colSpan={2} className="p-24 text-center text-[#4a5568] text-[10px] font-bold uppercase tracking-widest">Bu guruhda o'quvchilar yo'q</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
                    ...
                    {activeTab === 'ballar' && (
                        <div className="space-y-10 animate-in duration-500 zoom-in-95">
                            <h3 className="text-xl font-bold text-white uppercase tracking-tight">O'quvchilar Natijalari</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {groupStudents.map(s => {
                                    ...
                                    return (
                                        <div key={s.id} className="bg-[#141c27] border border-white/5 p-6 rounded-[2rem] hover:border-sky-300 dark:hover:border-sky-500 transition-all shadow-sm hover:shadow-xl hover:shadow-sky-500/5 group hover:-translate-y-1">
                                            <div className="flex items-center justify-between mb-6">
                                                <div className="flex-1 pr-3 truncate">
                                                    <h4 className="text-sm font-bold text-white uppercase tracking-tight group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">{s.name}</h4>
                                                    <button 
                                                        onClick={() => {
                                            ...
                                            <div className="space-y-3 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                                                {studentScores.map(sc => (
                                                    <div key={sc.id} className="flex items-center justify-between py-3 border-b border-dashed border-white/5 last:border-0">
                                                        <span className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">{sc.date}</span>
                                                        <span className="text-[10px] font-extrabold text-white bg-white/5 px-3 py-1.5 rounded-xl border border-white/5 transition-colors tabular-nums">{sc.value} B</span>
                                                    </div>
                                                ))}
                                                {studentScores.length === 0 && (
                                                    <div className="py-10 text-center flex flex-col items-center gap-4 animate-in fade-in duration-700">
                                                        <div className="w-12 h-12 bg-white/5 border border-dashed border-white/5 rounded-2xl flex items-center justify-center text-white/20">
                                                            <Award size={24} />
                                                        </div>
                                                        <p className="text-[10px] text-[#4a5568] font-bold uppercase tracking-widest italic opacity-60">Natijalar mavjud emas</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
            ...
            {isScoreModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setIsScoreModalOpen(false)}>
                    <div className="bg-[#1a2332] w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/5 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <div className="px-8 py-6 flex items-center justify-between border-b border-white/5 bg-white/[0.03]">
                            <div>
                                <h2 className="text-xl font-bold text-white uppercase tracking-tight">Ball qo'shish</h2>
                                <p className="text-[10px] font-bold text-[#4a5568] mt-1.5 uppercase tracking-widest">{students.find(s => s.id === selectedStudentForScore)?.name}</p>
                            </div>
                            <button onClick={() => setIsScoreModalOpen(false)} className="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-2xl text-gray-400 hover:text-white transition-all shadow-sm border border-transparent hover:border-white/5">
                                <XCircle size={22} />
                            </button>
                        </div>
                        <div className="p-8 space-y-5">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Ball miqdori</label>
                                <input type="number" placeholder="Masalan: 85" value={newScoreValue} onChange={e => setNewScoreValue(e.target.value)}
                                    className="w-full px-5 py-3.5 bg-white/5 border border-white/5 rounded-[1.25rem] text-sm font-bold focus:border-sky-500 transition-all outline-none text-white placeholder:text-gray-400/60" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Sana</label>
                                <input type="date" value={newScoreDate} onChange={e => setNewScoreDate(e.target.value)}
                                    className="w-full px-5 py-3.5 bg-white/5 border border-white/5 rounded-[1.25rem] text-sm font-bold focus:border-sky-500 transition-all outline-none text-white" />
                            </div>
                            <div className="space-y-2">
                                ...
                                <textarea placeholder="Darsdagi faollik..." value={newScoreComment} onChange={e => setNewScoreComment(e.target.value)}
                                    className="w-full px-5 py-3.5 bg-white/5 border border-white/5 rounded-[1.25rem] text-sm font-bold focus:border-sky-500 transition-all outline-none text-white placeholder:text-gray-400/60 min-h-[80px] resize-none" />
                            </div>
            ...
            {isAddStudentModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setIsAddStudentModalOpen(false)}>
                    <div className="bg-[#1a2332] w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/5 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <div className="px-8 py-6 flex items-center justify-between border-b border-white/5 bg-white/[0.03]">
                            <div>
                                <h2 className="text-xl font-bold text-white uppercase tracking-tight">O'quvchi qo'shish</h2>
                                <p className="text-[10px] font-bold text-[#4a5568] mt-1.5 uppercase tracking-widest">Guruhga biriktirish</p>
                            </div>
                            <button onClick={() => setIsAddStudentModalOpen(false)} className="w-10 h-10 flex items-center justify-center hover:bg-white/5 rounded-2xl text-gray-400 hover:text-white transition-all shadow-sm border border-transparent hover:border-white/5">
                                <XCircle size={22} />
                            </button>
                        </div>
                        <div className="p-8 space-y-6">
                            <div className="relative group">
                                ...
                                <input type="text" placeholder="Ism bo'yicha qidirish..." value={studentSearch} onChange={e => setStudentSearch(e.target.value)}
                                    className="w-full pl-11 pr-5 py-3.5 bg-white/5 border border-white/5 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 transition-all outline-none text-white placeholder:text-gray-400/60 shadow-inner" />
                            </div>
                            <div className="max-h-[350px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                                {availableStudents.map(s => (
                                    <button key={s.id} onClick={() => handleAddStudent(s.id)}
                                        className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-white/5 border border-transparent hover:border-sky-800/50 transition-all group overflow-hidden relative">
                                        <div className="flex items-center gap-4 relative z-10">
                                            ...
                                            <span className="text-sm font-bold text-white group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors uppercase tracking-tight">{s.name}</span>
                                        </div>
                                        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white/20 group-hover:text-sky-400 group-hover:bg-sky-900/50 transition-all relative z-10 border border-transparent group-hover:border-sky-800">
                                ...
                                {availableStudents.length === 0 && (
                                    <div className="py-16 text-center animate-in fade-in duration-500">
                                        <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-4 border border-dashed border-white/5">
                                            <Search size={24} className="text-gray-300" />
                                        </div>
                                        <p className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">O'quvchi topilmadi</p>
                                    </div>
                                )}
                            </div>
                        </div>
...
function StatCardV3({ label, value, subValue, icon, color }: any) {
    ...
    return (
        <div className="bg-[#141c27] p-6 rounded-[2rem] border border-white/5 shadow-sm transition-all hover:shadow-xl hover:shadow-sky-500/5 hover:-translate-y-1">
            <div className="flex items-start justify-between">
                <div className="space-y-2">
                    <p className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">{label}</p>
                    <h5 className="text-2xl font-extrabold text-white tracking-tight leading-none tabular-nums pt-1">{value}</h5>
                </div>
            ...
            <div className="pt-5 mt-5 border-t border-dashed border-white/5">
                <p className="text-[10px] font-bold text-[#4a5568] flex items-center gap-2 uppercase tracking-widest leading-none">
...
function TabButton({ label, icon, active, onClick }: any) {
    return (
        <button onClick={onClick} className={`px-8 py-5 text-xs font-bold uppercase tracking-widest flex items-center gap-3 transition-all relative shrink-0 ${active ? 'text-sky-600 dark:text-sky-400 bg-[#141c27] shadow-sm' : 'text-[#4a5568] hover:text-white'}`}>
    ...
    return (
        <div className="flex items-center gap-5 group">
            ...
            <div className="min-w-0 space-y-1">
                <p className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest leading-none">{label}</p>
