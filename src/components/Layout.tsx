      ...
      <header className="sticky top-0 z-50 border-b border-chiziq bg-sirt-2">
        ...
        <div className="h-[54px] flex items-center justify-between px-4 lg:px-6">
          <div className="flex items-center gap-4">
            ...
            {/* Bitta filialda ishlaydigan xodim — tanlagich o'rniga nomi. */}
            {user?.role !== 'SUPERADMIN' && user?.role !== 'SELLER' && user?.role !== 'ADMIN' && schools.length === 1 && (
            ...
            {/* Branch selector. ADMIN — barcha filiallar; bir nechta filialda ishlaydigan
                xodim (HR'da galochka qo'yilgan) — faqat o'z filiallari, server ham
                shularni qaytaradi. */}
            {(user?.role === 'ADMIN' || (user?.role !== 'SUPERADMIN' && user?.role !== 'SELLER' && schools.length > 1)) && (
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[200] lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[280px] bg-white dark:bg-slate-900 shadow-2xl flex flex-col animate-in slide-in-from-left duration-200">
            ...
            {/* Telefonda ham filial almashtirish: ustozlar ko'pincha telefondan kiradi. */}
            {(user?.role === 'ADMIN' || (user?.role !== 'SUPERADMIN' && user?.role !== 'SELLER' && schools.length > 1)) && (
              <div className="px-3 pt-3">
                <div className="relative">
                  ...
                  <select
                    aria-label="Filial"
                    value={selectedSchoolId === null ? '' : selectedSchoolId}
                    onChange={(e) => { setSelectedSchoolId(Number(e.target.value)); setMobileMenuOpen(false); }}
                    className="w-full pl-9 pr-8 py-2.5 bg-slate-50 dark:bg-gray-850 border border-chiziq rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-300 outline-none focus:border-brand appearance-none cursor-pointer"
                  >
                    <option value="" disabled>Filialni tanlang</option>
                    {user?.role === 'ADMIN' && <option value="0">To'liq o'quv markazi</option>}
                    {schools.map(school => (
                      <option key={school.id} value={school.id}>{school.name}</option>
                    ))}
                  </select>
