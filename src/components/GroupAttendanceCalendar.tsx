...
export default function GroupAttendanceCalendar({ group, attendances, selectedDate, onSelectDate, students }: GroupAttendanceCalendarProps) {
    ...
    const StatusChoicePopover = ({ date }: { date: string }) => (
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50 bg-[#1a2332] rounded-2xl shadow-2xl border border-white/5 p-2 flex flex-col gap-1 min-w-[170px] animate-in slide-in-from-top-2 duration-200">
            <button 
                onClick={() => handleBatchStatus(date, 'O\'tildi')}
                className="flex items-center gap-3 px-4 py-3 hover:bg-emerald-500/10 text-emerald-400 rounded-xl transition-all"
            >
            ...
            <button 
                ...
                className="flex items-center gap-3 px-4 py-3 hover:bg-rose-500/10 text-rose-400 rounded-xl transition-all"
            >
    ...
    return (
        <div className="bg-[#141c27] rounded-[2.5rem] border border-white/5 p-8 shadow-sm transition-all h-full flex flex-col relative overflow-visible">
            <div className="flex items-center justify-between mb-8 px-2">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={handlePrevMonth} className="p-2 hover:bg-white/5 rounded-xl text-[#4a5568] transition-all"><ChevronLeft size={18} /></button>
                    <button onClick={handleNextMonth} className="p-2 hover:bg-white/5 rounded-xl text-[#4a5568] transition-all"><ChevronRight size={18} /></button>
                </div>
            </div>

            <div className="grid grid-cols-7 gap-2 flex-grow">
                {['Du', 'Se', 'Ch', 'Pa', 'Ju', 'Sh', 'Ya'].map(d => (
                    <div key={d} className="text-center text-[9px] font-extrabold text-white/15 uppercase tracking-widest pb-3">{d}</div>
                ))}
                ...
                {Array(daysInMonth).fill(0).map((_, i) => {
                    ...
                    const bg = status === 'conducted' ? 'bg-emerald-500/10 text-emerald-400' :
                               status === 'not_conducted' ? 'bg-rose-500/10 text-rose-400' :
                               status === 'unmarked' ? 'bg-amber-400/10 text-amber-400' : 'bg-transparent';
                    const border = isSelected ? 'border-sky-500 ring-2 ring-sky-500 ring-offset-2 ring-offset-[#141c27]' : 'border-sky-500/10';
                    const dotColor = status === 'conducted' ? 'bg-emerald-500' : 
                                     status === 'not_conducted' ? 'bg-rose-500' : 
                                     status === 'unmarked' ? 'bg-amber-400' : 'bg-white/20';
                    ...
                    return (
                        <div key={d} className="relative">
                            ...
                            >
                                <span className={`text-[10px] font-bold ${isSelected ? 'text-sky-400 scale-110' : (isLesson ? 'text-white/60' : 'text-white/15')}`}>
                                    {d}
                                </span>
