...
import { Search, Plus, X, Users, Layers, ChevronRight, SlidersHorizontal, BookOpen, Edit, Trash2 } from 'lucide-react';
...
export default function Courses() {
    ...
    // Auto-calculate 2 hours for Group schedule
    React.useEffect(() => {
    ...
    const handleAddGroup = async (e: React.FormEvent) => {
        ...
        if (!newGroup.courseId) {
            showNotification("Iltimos, guruh uchun kursni tanlang.", "error");
            return;
        }
        try {
            ...
            setNewGroup({ name: '', teacherId: 0, startTime: '', endTime: '', days: 'TOQ', room: '', courseId: '', syllabusId: '' });
    ...
    const handleGroupCourseChange = (cId: number) => {
    ...
    const handleSaveCourse = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!courseForm.name.trim() || !courseForm.price.trim() || !courseForm.syllabusId) {
            showNotification("Barcha maydonlarni to'ldiring va o'quv programmasini tanlang.", "error");
            return;
        }
        ...
        try {
            ...
            const courseData = {
                ...
                syllabusId: Number(courseForm.syllabusId)
            };

            if (editingCourse) {
                await updateCourse(editingCourse.id, courseData);
                showNotification("Kurs muvaffaqiyatli yangilandi", "success");
            } else {
                await addCourse(courseData);
                showNotification("Kurs muvaffaqiyatli qo'shildi", "success");
            }
            setIsCourseModalOpen(false);
        } catch (err) {
            showNotification("Kursni saqlashda xatolik yuz berdi", "error");
        } finally {
    ...
    const handleDeleteCourse = async (id: number) => {
        if (window.confirm("Haqiqatan ham ushbu kursni o'chirmoqchisiz? U bilan bog'liq guruhlar ta'sirlanishi mumkin.")) {
            try {
                await deleteCourse(id);
                showNotification("Kurs o'chirildi", "success");
            } catch (err) {
    ...
    const getSyllabusName = (id: number | null | undefined) => {
        if (!id) return "Programma biriktirilmagan";
        return syllabuses.find(s => s.id === id)?.name || "Noma'lum programma";
    };
    ...
    const activeCourses = courses.filter(c => c.name !== 'birinchi');

    return (
        <div className="space-y-6">
            {/* Navigation Tabs */}
            <div className="flex border-b border-gray-100 dark:border-gray-750 gap-6">
                <button
            ...
            {activeTab === 'groups' ? (
                                ...
                                return (
                                    <div key={group.id} onClick={() => navigate(`/courses/${group.id}`)}
                                        ...
                                        <div className="flex-1 mb-4">
                                            ...
                                            {getCourseName(group.courseId) !== 'birinchi' && (
                                ...
                                <form onSubmit={handleAddGroup} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select required className={inp} value={newGroup.courseId} onChange={e => handleGroupCourseChange(Number(e.target.value))}>
                ...
                <>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700/50 shadow-sm overflow-hidden">
                        <div className="px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h1 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">Kurslar</h1>
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
                                        {activeCourses.length} • Ro'yxatdan o'tgan kurslar
                                    </p>
                                </div>
                            </div>
                            <button
                    ...
                    {activeCourses.length === 0 ? (
                        <div className="py-24 text-center bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700/50 border-dashed">
                            <BookOpen size={40} className="mx-auto text-gray-200 dark:text-gray-600 mb-3" />
                            <p className="text-sm font-bold text-gray-400">Hozircha kurslar mavjud emas</p>
                            <button onClick={() => handleOpenCourseModal()}
                                className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-[#1b6b6b] text-white text-xs font-extrabold uppercase tracking-widest rounded-xl cursor-pointer">
                                <Plus size={13} /> Yangi Kurs yaratish
                            </button>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {activeCourses.map(course => (
                                <div key={course.id}
                                    className="group bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-sm p-5 flex flex-col justify-between hover:shadow-xl hover:border-[#1b6b6b]/40 dark:hover:border-[#1b6b6b]/40 transition-all duration-300">
                                    <div>
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="w-10 h-10 rounded-xl bg-[#1b6b6b]/10 dark:bg-[#1b6b6b]/20 flex items-center justify-center text-[#1b6b6b] font-black shrink-0">
                                                <BookOpen size={18} />
                                            </div>
                                            <div className="flex gap-1">
                                                <button onClick={() => handleOpenCourseModal(course)} className="p-1.5 rounded-lg text-gray-400 hover:text-[#1b6b6b] hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                                                    <Edit size={14} />
                                                </button>
                                                <button onClick={() => handleDeleteCourse(course.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-rose-500 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                                                    <Trash2 size={14} />
                                                </button>
                                            </div>
                                        </div>
                                        <h3 className="font-black text-gray-900 dark:text-white uppercase tracking-tight mb-2 line-clamp-1">{course.name}</h3>
                                        <div className="space-y-1 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">
                                            <p className="flex items-center gap-1.5 text-gray-900 dark:text-white text-xs font-black">
                                                {String(course.price).replace(/\B(?=(\d{3})+(?!\d))/g, " ")} UZS
                                            </p>
                                        </div>
                                    </div>
                                    <div className="pt-3 border-t border-dashed border-gray-100 dark:border-gray-700">
                                        <span className="block text-[8px] text-gray-400 uppercase tracking-widest mb-1">O'quv Programmasi</span>
                                        <span className="text-xs font-black text-gray-900 dark:text-white truncate block">
                                            {getSyllabusName(course.syllabusId)}
                                        </span>
                                    </div>
                                </div>
                            ))}
                            ...
                            <div className="relative bg-white dark:bg-gray-800 rounded-[2rem] border border-gray-100 dark:border-gray-700/50 shadow-2xl w-full max-w-lg p-8 max-h-[90vh] overflow-y-auto">
                                <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-50 dark:border-gray-700/50">
                                    <div>
                                        ...
                                        <p className="text-[10px] font-bold text-[#1b6b6b] uppercase tracking-widest mt-0.5">Kurs ma'lumotlarini to'ldiring</p>
                                    </div>
                                ...
                                <form onSubmit={handleSaveCourse} className="space-y-4">
                                    <div>
                                        ...
                                        <input required type="text" placeholder="Matematika" className={inp} value={courseForm.name} onChange={e => setCourseForm({ ...courseForm, name: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className={lbl}>Narxi (UZS/oy) *</label>
                                        <input required type="number" placeholder="400000" className={inp} value={courseForm.price} onChange={e => setCourseForm({ ...courseForm, price: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className={lbl}>O'quv Programmasi (Syllabus) *</label>
                                        <select required className={inp} value={courseForm.syllabusId} onChange={e => setCourseForm({ ...courseForm, syllabusId: e.target.value === '' ? '' : Number(e.target.value) })}>
                                            <option value="" disabled>O'quv programmasini tanlang *</option>
                                            {syllabuses.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                        </select>
                                    </div>
                                    <div className="flex gap-3 pt-2">
                                        ...
                                        <button type="submit" disabled={isProcessingCourse}
                                            className="flex-1 py-3 bg-[#1b6b6b] hover:bg-[#155252] text-white text-xs font-extrabold uppercase tracking-widest rounded-2xl shadow-lg shadow-[#1b6b6b]/20 transition-all cursor-pointer disabled:opacity-50">
                                            {isProcessingCourse ? t('saving') : t('save')}
                                        </button>
                                    </div>
                                </form>
                            </div>
