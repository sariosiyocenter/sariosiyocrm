...
import { Lock, Unlock, CheckCircle2, AlertTriangle, RefreshCw, KeyRound, ArrowRight, BookOpen } from 'lucide-react';
...
interface Qoida { mavzu: string; tur: string; kerak: number; bor: number; yetadi: boolean }
...
export default function TuzilmaTab({ exam, yangila, bolimgaOt }: { exam: ImtihonTafsil; yangila: () => Promise<any>; bolimgaOt: (b: BolimId) => void }) {
  ...
  const tekshir = useCallback(() => soro<{ bloklar: any[] }>('GET', `exams/${exam.id}/check`).then(r => setBank(r.bloklar)).catch(e => showNotification(e.message, 'error')), [exam.id, soro, showNotification]);
  useEffect(() => { if (!exam.lockedAt) tekshir(); }, [exam.lockedAt, tekshir]);

  const qulfla = async () => {
    if (!(await confirm("Savollar bankdan tanlanib, variantlar yasaladi. Shundan keyin imtihon tuzilmasi o'zgarmaydi (natija kiritilmaguncha qulfni ochish mumkin). Davom etilsinmi?"))) return;
    setBand(true);
    setKamchilik(null);
    try {
  ...
  const hammasiYetadi = bank?.every(b => b.qoidalar.every(q => q.yetadi));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 space-y-4">
        {!exam.lockedAt ? (
          <Karta sarlavha="Savollar banki yetadimi" izoh={s.sessionQuestions === 'alohida' && s.sessions.length > 1 ? `Har smenaga alohida savol: kerakli son ${s.sessions.length} barobar` : 'Faqat faol va to\'liq to\'ldirilgan savollar hisoblanadi'}
            ...
            {!bank ? <Yuklanmoqda /> : (
              <div className="space-y-3">
                {bank.map((b, i) => (
                  <div key={i}>
                    ...
                    <div className="rounded-xl border border-chiziq divide-y divide-chiziq">
                      {b.qoidalar.map((q, j) => (
                        <div key={j} className="flex items-center justify-between gap-2 px-3 py-2 text-[12.5px]">
                          <span className="text-matn">{q.mavzu || 'Istalgan mavzu'} <span className="text-matn-xira">· {TUR[q.tur]}</span></span>
                ...
                {!hammasiYetadi && (
                  <div className="flex flex-wrap items-center gap-2 rounded-xl bg-ogoh-fon border border-ogoh/25 px-3 py-2.5 text-[12.5px] text-matn">
          ...
          <Karta sarlavha="Variantlar tayyor" izoh={`Qulflangan: ${new Date(exam.lockedAt).toLocaleString('uz')}`}
            ...
            {exam.blocks.map((b, i) => (
              <div key={i} className="rounded-xl border border-chiziq p-3">
                ...
                <ul className="mt-1.5 text-[12.5px] text-matn-sokin space-y-0.5">
                  {b.topicRules.map((r, j) => <li key={j}>{r.topic || 'Istalgan mavzu'} — {r.count} ta {TUR[r.type || 'yopiq']}{r.difficulty ? `, qiyinlik ${r.difficulty}` : ''}{r.points != null ? `, ${r.points} ball` : ''}</li>)}
                </ul>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-4">
        <Karta sarlavha="Keyingi qadam">
          {!exam.lockedAt ? (
            <div className="space-y-3">
              <p className="text-[12.5px] text-matn-sokin">Savollar tayyor bo'lgach — qulflang. Variantlar serverda yasaladi, kalit faqat ruxsati borlarga ko'rinadi.</p>
              {tahrir && <Tugma turi="asosiy" className="w-full" ikonka={<Lock size={14} />} yuklanmoqda={band} disabled={bank !== null && !hammasiYetadi} onClick={qulfla}>Savollarni qulflash</Tugma>}
            </div>
          ) : (
            <div className="space-y-3">
        ...
        <Karta sarlavha="Sozlamalar">
          <dl className="text-[12.5px] space-y-1.5">
            ...
            <Qator k="Aralashtirish" v={[s.shuffleQuestions && 'savollar', s.shuffleOptions && 'javoblar'].filter(Boolean).join(', ') || "yo'q"} />
      ...
      {kalit && <KalitOynasi examId={exam.id} onYop={ozgardi => { setKalit(false); if (ozgardi) yangila(); }} />}
    </div>
  );
}
