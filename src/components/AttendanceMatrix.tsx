...
export default function AttendanceMatrix({ group, students, attendances }: AttendanceMatrixProps) {
    ...
    const getStatusColor = (studentId: number, date: string) => {
        ...
        if (!att) {
            ...
            return 'bg-white/5 border border-dashed border-white/10'; // Future
        }
        ...
        if (att.status === 'Dars bo\'lmadi') return 'bg-white/30';
    ...
    return (
        <div className="bg-[#141c27] rounded-[2.5rem] shadow-sm border border-white/5 transition-all">
            <div className="overflow-x-auto custom-scrollbar min-h-[500px] p-2">
                <table className="w-full text-left border-separate border-spacing-0">
                    <thead>
                        <tr className="bg-white/[0.03]">
                            <th className="sticky left-0 z-30 bg-[#1a2332] p-6 text-[10px] font-bold text-[#4a5568] uppercase tracking-widest border-b border-r border-white/5 min-w-[200px]">O'quvchi</th>
                            {lessonDates.map(date => (
                                <th key={date} className="p-6 text-center text-[10px] font-bold text-[#4a5568] uppercase tracking-widest border-b border-white/5">{getDayLabel(date)}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                        {students.map(student => (
                            <tr key={student.id} className="hover:bg-white/[0.02] transition-colors">
                                <td className="sticky left-0 z-20 bg-[#141c27] p-6 border-r border-white/5">
                                    <p className="text-[11px] font-black text-white uppercase tracking-tighter truncate">{student.name}</p>
                                </td>
                                {lessonDates.map(date => (
                                    <td key={date} className="p-4 text-center relative overflow-visible">
                                        ...
                                        {activePopover?.studentId === student.id && activePopover?.date === date && (
                                            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50 bg-[#1a2332] rounded-2xl shadow-2xl border border-white/5 p-2 flex flex-col gap-1 min-w-[140px] animate-in slide-in-from-top-2 duration-200">
                                                <button onClick={() => handleStatusSelect(student.id, date, 'Keldi')} className="flex items-center gap-3 px-4 py-3 hover:bg-emerald-500/10 text-emerald-400 rounded-xl transition-all">
                                                ...
                                                <button onClick={() => handleStatusSelect(student.id, date, 'Kelmapdi')} className="flex items-center gap-3 px-4 py-3 hover:bg-rose-500/10 text-rose-400 rounded-xl transition-all">
                                                ...
                                                <button onClick={() => handleStatusSelect(student.id, date, 'Sababli')} className="flex items-center gap-3 px-4 py-3 hover:bg-sky-500/10 text-sky-400 rounded-xl transition-all">
                                                ...
                                                <button onClick={() => handleStatusSelect(student.id, date, 'Dars bo\'lmadi')} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 text-[#4a5568] rounded-xl transition-all">
                                                    <div className="w-5 h-5 bg-white/30 rounded-lg flex items-center justify-center text-white font-bold text-[10px]">X</div>
                                                    <span className="text-[10px] font-bold uppercase tracking-widest leading-none">Dars bo'lmadi</span>
                                                </button>
                                                <div className="h-px bg-white/5 my-1 px-2" />
                                                <button onClick={() => setActivePopover(null)} className="flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-[#4a5568] rounded-xl transition-all">
                                                    <XCircle size={14} />
            ...
            <div className="p-8 bg-white/[0.02] border-t border-dashed border-white/5 flex flex-wrap gap-6 justify-center">
                ...
                <StatusLink color="bg-white/30" label="X - Dars bo'lmadi" />
                <StatusLink color="bg-amber-400" label="Belgilanmagan" />
                <StatusLink color="bg-white/5 border-white/10" label="Rejadagi" />
            </div>
        </div>
    );
}
...
function StatusLink({ color, label }: { color: string, label: string }) {
    return (
        <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-bold text-[#4a5568] uppercase tracking-widest">{label}</span>
        </div>
    );
}
