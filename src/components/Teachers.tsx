        ...
        <div className="space-y-6">
            ...
            <div className="bg-sirt rounded-2xl border border-chiziq shadow-sm overflow-hidden">
                <div className="px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg font-black text-matn tracking-tight">{t('teachers_title')}</h1>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="pl-9 pr-4 py-2.5 bg-ichki border border-chiziq rounded-xl text-xs font-bold text-matn outline-none focus:border-violet-500 transition-all w-52"
                            />
                        </div>
                        <button
                            onClick={() => setShowFilters(!showFilters)}
                            className={`w-10 h-10 flex items-center justify-center rounded-xl border transition-all cursor-pointer ${showFilters ? 'bg-violet-600 border-violet-600 text-white' : 'bg-ichki border-chiziq text-matn-xira hover:border-violet-400 hover:text-violet-500'}`}
                        >
                ...
                {showFilters && (
                    <div className="px-6 pb-5 pt-4 border-t border-chiziq-mayin/50 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        {[
                            ...
                            <div key={f.key}>
                                <label className="text-[11px] font-extrabold text-matn-xira block mb-1.5">{f.label}</label>
                                <select value={(filters as any)[f.key]} onChange={e => setFilters({ ...filters, [f.key]: e.target.value })}
                                    className="w-full px-3 py-2 bg-ichki border border-chiziq rounded-xl text-[11px] font-bold text-gray-700 dark:text-white outline-none focus:border-violet-400 transition-all cursor-pointer">
                        ...
                        <div>
                            ...
                            <input type="number" placeholder="1990" value={filters.minBirthYear} onChange={e => setFilters({ ...filters, minBirthYear: e.target.value })}
                                className="w-full px-3 py-2 bg-ichki border border-chiziq rounded-xl text-[11px] font-bold text-gray-700 dark:text-white outline-none focus:border-violet-400 transition-all" />
                        </div>
                        <div>
            ...
            {filteredTeachers.length === 0 ? (
                <div className="py-24 text-center bg-sirt rounded-2xl border border-chiziq border-dashed">
                    ...
                    <p className="text-sm font-bold text-matn-xira">{t('teachers_not_found')}</p>
                    ...
                    {filteredTeachers.map(teacher => (
                        <div key={teacher.id} onClick={() => navigate(`/teachers/${teacher.id}`)}
                            ...
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${teacher.status === 'Faol' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/40' : 'bg-gray-50 text-matn-xira border-gray-100 dark:bg-gray-900/50 dark:border-gray-800'}`}>
                                    {teacher.status}
                                </span>
                            </div>

                            <div className="flex-1 mb-4">
                                <h3 className="font-black text-matn tracking-tight group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors line-clamp-1">{teacher.name}</h3>
                                <div className="flex items-center gap-1.5 text-[12px] text-matn-xira font-bold mt-1">
                                    <Phone size={11} /> {teacher.phone}
                                </div>
                            </div>

                            <div className="flex items-center justify-between pt-3 border-t border-chiziq-mayin/50">
                                <span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${salaryColors[teacher.salaryType] || salaryColors.FIXED}`}>
                                    {salaryTypeLabels[teacher.salaryType] || 'Fiks'}
                                </span>
                                <p className="text-sm font-black text-matn tabular-nums">
                                    {teacher.salaryType === 'KPI' ? `${teacher.sharePercentage}%` : teacher.salary.toLocaleString()}
                                    {teacher.salaryType !== 'KPI' && <span className="text-[11px] text-matn-xira ml-1">UZS</span>}
                                </p>
                            </div>
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-[200] flex items-start sm:items-center justify-center overflow-y-auto p-4">
                    ...
                    <div className="relative bg-sirt rounded-[2rem] border border-chiziq shadow-2xl w-full max-w-lg p-8 max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-6 pb-4 border-b border-chiziq-mayin/50">
                            <div>
                                <h3 className="text-lg font-black text-matn tracking-tight">{t('new_teacher_title')}</h3>
                                <p className="text-[11px] font-bold text-violet-600 mt-0.5">{t('teacher_details_subtitle')}</p>
                            </div>
                            <button aria-label="Yopish" onClick={() => setIsModalOpen(false)} className="w-9 h-9 flex items-center justify-center text-matn-xira hover:bg-gray-55 dark:hover:bg-gray-700 rounded-xl cursor-pointer"><X size={18} /></button>
                        </div>
                                ...
                                <div className="flex gap-2">
                                    {(['FIXED', 'KPI', 'FIXED_KPI'] as const).map(type => (
                                        <button key={type} type="button" onClick={() => setNewTeacher({ ...newTeacher, salaryType: type })}
                                            className={`flex-1 py-2.5 rounded-xl text-[11px] font-extrabold border transition-all cursor-pointer ${newTeacher.salaryType === type ? 'bg-brand border-brand text-white' : 'bg-ichki border-chiziq text-matn-sokin hover:border-brand/30'}`}>
