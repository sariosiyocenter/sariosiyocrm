            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-800/50 shadow-sm overflow-hidden">
                <div className="px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-lg font-black text-gray-900 dark:text-white tracking-tight">{t('teachers_title')}</h1>
                ...
                {showFilters && (
                    <div className="px-6 pb-5 pt-4 border-t border-gray-50 dark:border-gray-800/50 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        {[
                            ...
                            <div key={f.key}>
                                <label className="text-[11px] font-extrabold text-gray-400 block mb-1.5">{f.label}</label>
                    ...
                    {filteredTeachers.map(teacher => (
                        <div key={teacher.id} onClick={() => navigate(`/teachers/${teacher.id}`)}
                            ...
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${teacher.status === 'Faol' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/40' : 'bg-gray-50 text-gray-400 border-gray-100 dark:bg-gray-900/50 dark:border-gray-800'}`}>
                                    {teacher.status}
                                </span>
                            </div>

                            <div className="flex-1 mb-4">
                                <h3 className="font-black text-gray-900 dark:text-white tracking-tight group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors line-clamp-1">{teacher.name}</h3>
                                <div className="flex items-center gap-1.5 text-[12px] text-gray-400 font-bold mt-1">
                            ...
                            <div className="flex items-center justify-between pt-3 border-t border-gray-50 dark:border-gray-800/50">
                                ...
                                <p className="text-sm font-black text-gray-900 dark:text-white tabular-nums">
                                    ...
                                    {teacher.salaryType !== 'KPI' && <span className="text-[11px] text-gray-400 ml-1">UZS</span>}
                                </p>
                            </div>
                        </div>
                    ))}
                                ...
                                <div className="flex gap-2">
                                    {(['FIXED', 'KPI', 'FIXED_KPI'] as const).map(type => (
                                        <button key={type} type="button" onClick={() => setNewTeacher({ ...newTeacher, salaryType: type })}
                                            className={`flex-1 py-2.5 rounded-xl text-[11px] font-extrabold border transition-all cursor-pointer ${newTeacher.salaryType === type ? 'bg-[#1b6b6b] border-[#1b6b6b] text-white' : 'bg-gray-55 dark:bg-gray-900/50 border-gray-100 dark:border-gray-800 text-gray-500 hover:border-[#1b6b6b]/30'}`}>
