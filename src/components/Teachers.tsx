...
export default function Teachers() {
    ...
    return (
        <div className="p-4 sm:p-8 space-y-8 animate-in fade-in duration-700">
            {/* Header section */}
            <div className="bg-[#141c27] rounded-[2.5rem] shadow-xl shadow-black/20 border border-white/5 overflow-hidden transition-all duration-300">
                <div className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h1 className="text-2xl font-extrabold text-white uppercase tracking-tight">Ustozlar</h1>
                            <p className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest mt-1">Jami {teachers.length} ta o'qituvchi</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="relative group flex-1 md:w-80">
                            ...
                            <input 
                                ...
                                className="w-full bg-white/5 border border-white/10 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 rounded-2xl pl-12 pr-6 py-3.5 text-[10px] font-extrabold uppercase tracking-widest text-white placeholder:text-[#4a5568] outline-none transition-all shadow-inner"
                            />
                        </div>
                        <button 
                            onClick={() => setShowFilters(!showFilters)}
                            className={`w-14 h-14 flex items-center justify-center rounded-2xl border transition-all duration-300 ${showFilters ? 'bg-violet-600 border-violet-600 text-white shadow-lg shadow-violet-500/30' : 'bg-[#141c27] border-white/5 text-gray-500 hover:border-violet-500 hover:text-violet-500'}`}
                        >
                            <Filter size={20} />
                        </button>
                    </div>
                    ...
                    <div className="px-8 pb-8 pt-2 border-t border-white/5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6 animate-in slide-in-from-top-4 duration-500">
                        <div className="space-y-3">
                            ...
                            <select 
                                ...
                                className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-bold uppercase tracking-widest outline-none focus:border-violet-500 transition-all text-white appearance-none cursor-pointer"
                            >
                        ...
                        <div className="space-y-3">
                            <label className="text-[9px] font-extrabold text-[#4a5568] uppercase tracking-widest ml-1">Ish haqi turi</label>
                            <select 
                        ...
                        <div className="space-y-3">
                            <label className="text-[9px] font-extrabold text-[#4a5568] uppercase tracking-widest ml-1">Tug'ilgan yili (min)</label>
                            <input 
                                type="number"
                                placeholder="1990"
                                value={filters.minBirthYear}
                                onChange={e => setFilters({...filters, minBirthYear: e.target.value})}
                                className="w-full px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-bold uppercase tracking-widest outline-none focus:border-violet-500 transition-all text-white"
                            />
                        </div>
                        <div className="space-y-3">
                            <label className="text-[9px] font-extrabold text-[#4a5568] uppercase tracking-widest ml-1">Tug'ilgan yili (max)</label>
                            <input 
                                type="number"
                                placeholder="2005"
                        ...
                        <div className="space-y-3">
                            <label className="text-[9px] font-extrabold text-[#4a5568] uppercase tracking-widest ml-1">Ishga kirgan vaqti</label>
                            <select 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {filteredTeachers.map((teacher, idx) => (
                    <div
                        ...
                        className="bg-[#141c27] p-8 rounded-[2.5rem] border border-white/5 hover:border-violet-300 dark:hover:border-violet-500 group cursor-pointer transition-all shadow-sm hover:shadow-2xl hover:shadow-violet-500/10 hover:-translate-y-2 flex flex-col relative overflow-hidden animate-in zoom-in-95 duration-300"
                    ...
                    >
                        ...
                        <div className="flex items-start justify-between mb-8 relative z-10">
                            ...
                            <span className={`text-[9px] font-extrabold px-3 py-1.5 rounded-xl uppercase tracking-widest border shadow-sm transition-colors group-hover:bg-opacity-100 group-hover:text-white ${teacher.status === 'Faol' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-800/50 group-hover:bg-emerald-600 group-hover:border-emerald-600' : 'bg-white/5 text-[#4a5568] border-white/5'}`}>
                                {teacher.status}
                            </span>
                        </div>
                        
                        <div className="flex-1 mb-6 relative z-10">
                            <h3 className="text-lg font-bold text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors uppercase tracking-tight line-clamp-1" title={teacher.name}>{teacher.name}</h3>
                            <div className="flex items-center gap-2 text-[10px] font-bold text-[#4a5568] mt-2 uppercase tracking-widest">
                                <Phone size={14} className="text-violet-500" />
                                <span className="tabular-nums">{teacher.phone}</span>
                            </div>
                        </div>

                        <div className="flex items-center justify-between pt-6 border-t border-dashed border-white/5 mt-auto relative z-10">
                            <div className="px-3 py-1.5 bg-white/5 rounded-xl border border-white/5 text-[10px] font-bold text-[#4a5568] uppercase tracking-widest">
                                {salaryTypeLabels[teacher.salaryType] || 'Fiks'}
                            </div>
                            <p className="text-sm font-extrabold text-white tabular-nums tracking-tight">
                                {teacher.salaryType === 'KPI' ? `${teacher.sharePercentage}%` : teacher.salary.toLocaleString()} <span className="text-[10px] text-[#4a5568] font-bold uppercase tracking-widest ml-1">{teacher.salaryType === 'KPI' ? '' : 'UZS'}</span>
                            </p>
                        </div>
                    </div>
                ))}
            ...
            {filteredTeachers.length === 0 && (
                <div className="py-20 text-center bg-[#141c27] border border-white/5 border-dashed rounded-xl">
                    <GraduationCap className="w-12 h-12 text-gray-600 mx-auto mb-3" />
                    <p className="text-sm font-semibold text-[#4a5568]">O'qituvchilar topilmadi</p>
                </div>
            )}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-[#1a2332] w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white/5" onClick={e => e.stopPropagation()}>
                        <div className="px-10 py-8 flex items-center justify-between border-b border-white/5 bg-white/[0.03]">
                            <div>
                                <h2 className="text-2xl font-bold text-white uppercase tracking-tight">Yangi Ustoz</h2>
                                <p className="text-[10px] font-bold text-[#4a5568] mt-1.5 uppercase tracking-widest">Yangi xodim ma'lumotlarini kiriting</p>
                            </div>
                        ...
                        <form onSubmit={handleAddTeacher} className="p-10 space-y-8">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    ...
                                    <input required type="text" placeholder="Masalan: Asror" className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all text-white shadow-inner"
                                ...
                                <div className="space-y-3">
                                    ...
                                    <input required type="tel" placeholder="+998" className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all text-white shadow-inner"
                                        value={newTeacher.phone} onChange={e => setNewTeacher({ ...newTeacher, phone: e.target.value })} />
                                </div>
                            </div>
                            
                            <div className="space-y-4">
                                <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Ish Haqini Hisoblash Turi</label>
                                    ...
                                    {(['FIXED', 'KPI', 'FIXED_KPI'] as const).map(type => (
                                        <button
                                            ...
                                            className={`flex-1 py-4 rounded-[1.25rem] text-[10px] font-extrabold uppercase tracking-widest transition-all border shadow-sm ${newTeacher.salaryType === type
                                                ...
                                                : 'bg-white/5 border-white/10 text-[#4a5568] hover:bg-white/5'
                                                }`}
                                        >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                {(newTeacher.salaryType === 'FIXED' || newTeacher.salaryType === 'FIXED_KPI') && (
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Fiks Oylik (UZS) <span className="text-rose-500">*</span></label>
                                        <input required type="number" placeholder="5 000 000" className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all text-white shadow-inner"
                                ...
                                {(newTeacher.salaryType === 'KPI' || newTeacher.salaryType === 'FIXED_KPI') && (
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Ulush Foizi (%) <span className="text-rose-500">*</span></label>
                                        <input required type="number" placeholder="40" min="0" max="100" step="0.1" className="w-full px-6 py-4 bg-emerald-50/50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all text-emerald-400 shadow-inner"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Tug'ilgan Sanasi <span className="text-rose-500">*</span></label>
                                    <input required type="date" className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all text-white shadow-inner"
                                        value={newTeacher.birthDate} onChange={e => setNewTeacher({ ...newTeacher, birthDate: e.target.value })} />
                                </div>
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold text-[#4a5568] uppercase tracking-widest ml-1">Ishga Kirgan Sanasi <span className="text-rose-500">*</span></label>
                                    <input required type="date" className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-[1.25rem] text-xs font-bold uppercase tracking-widest focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none transition-all text-white shadow-inner"
                            ...
                            <div className="pt-10 flex items-center justify-end gap-5 mt-6 border-t border-dashed border-white/10">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-4 bg-white/5 border border-white/10 text-[#4a5568] rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-white/5 transition-all">
                                    Bekor Qilish
                                </button>
